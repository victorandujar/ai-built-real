...
export function enhanceSelects(form: HTMLFormElement) {
  for (const select of form.querySelectorAll<HTMLSelectElement>('select')) {
    const label = form.querySelector<HTMLLabelElement>(`label[for="${select.id}"]`)!;
    ...
    chevron.className = 'choice-chevron'; chevron.textContent = '⌄'; chevron.setAttribute('aria-hidden','true');
    trigger.append(value, chevron);
    const menu = document.createElement('div');
    menu.className = 'choice-menu'; menu.id = `${select.id}-options`;
    ...
    const items = options.map((option, index) => {
      ...
      item.className = 'choice-option'; item.id = `${select.id}-option-${index}`;
      item.setAttribute('role','option');
      const title = document.createElement('span'); title.textContent = option.text;
      const mark = document.createElement('span'); mark.textContent = '✓'; mark.setAttribute('aria-hidden','true');
      item.append(title,mark); menu.append(item);
    ...
    function sync() {
      ...
      items.forEach((item,i)=>item.setAttribute('aria-selected',String(options[i].value===select.value)));
    }
    ...
    function open() {
      ...
      highlight(Math.max(0,options.findIndex(o=>o.value===select.value)));
    }
    ...
    trigger.addEventListener('keydown',e=>{
      ...
      if(['ArrowDown','ArrowUp','Home','End','Enter',' '].includes(e.key)) {
        ...
        const now=Date.now(); search=(now-lastKey>650?'':search)+e.key.toLowerCase(); lastKey=now;
        const match=options.findIndex(o=>o.text.toLowerCase().startsWith(search)); if(match>=0)highlight(match);
      }
    });
    wrap.addEventListener('focusout',e=>{if(!wrap.contains(e.relatedTarget as Node)){close();select.dispatchEvent(new Event('blur'));}});
...
export function focusField(field: HTMLElement) {
  const target = field instanceof HTMLSelectElement ? document.getElementById(`${field.id}-choice`) : field;
  target?.focus();
}
